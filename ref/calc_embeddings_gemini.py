...
import sqlite3
import embeddings_gemini
import vector_db

CHUNKS_DB_PATH = "../database/chunks.db"
EMBEDDINGS_DB_PATH = "../database/embeddings_gemini.db"
...
for collection in data.keys():
    ...
    for i in range(0, len(items), N):
        ...
        vectors = embeddings_gemini.get_embedding(chunks)
...
for collection, items in records.items():
    col_db = vector_db.VectorDB(EMBEDDINGS_DB_PATH, collection, embeddings_gemini.DIMENSION)
    col_db.delete_all()
    col_db.save(items)
